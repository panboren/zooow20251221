...
4. 着色器 - GPU 编程</pre>
      ...
      <div class="demo-section">
        ...
        <div class="button-group">
          ...
          <button class="btn btn-secondary" @click="nextLesson" :disabled="currentLesson === 12">
            下一课 →
          </button>
        </div>
...
async function runLesson13(ti: any) {
  try {
    ...
    // 创建庆祝效果
    const particles = ti.Vector.field(2, ti.f32, [200])
    const velocities = ti.Vector.field(2, ti.f32, [200])
    const colors = ti.Vector.field(3, ti.f32, [200])
    const pixels = ti.Vector.field(4, ti.f32, [IMAGE_SIZE, IMAGE_SIZE])
    ...
    const updateKernel = ti.kernel(function update() {
      for (let i of range(200)) {
        ...
        if (pos[0] < 0.0 || pos[0] > 1.0) vel[0] = -vel[0]
        if (pos[1] < 0.0 || pos[1] > 1.0) vel[1] = -vel[1]
    ...
    const renderKernel = ti.kernel(function render() {
      ...
      for (let p of range(200)) {
        ...
        if (px >= 1.0 && px < IMAGE_SIZE - 1.0 && py >= 1.0 && py < IMAGE_SIZE - 1.0) {
          pixels[[px, py]] = [col[0], col[1], col[2], 1.0]
          pixels[[px + 1.0, py]] = [col[0] * 0.7, col[1] * 0.7, col[2] * 0.7, 1.0]
          pixels[[px - 1.0, py]] = [col[0] * 0.7, col[1] * 0.7, col[2] * 0.7, 1.0]
          pixels[[px, py + 1.0]] = [col[0] * 0.7, col[1] * 0.7, col[2] * 0.7, 1.0]
          pixels[[px, py - 1.0]] = [col[0] * 0.7, col[1] * 0.7, col[2] * 0.7, 1.0]
        }
      }
    })
    ...
    function animateFrame() {
      ...
      tiCanvas.setImage(pixels)
      frameCount++
      status.value = `🎉 恭喜！庆祝粒子 | 帧: ${frameCount}`
      animationId = requestAnimationFrame(animateFrame)
    }

    isAnimating.value = true
    animateFrame()
    result.value = `🎉 恭喜完成所有13课课程！你已学会：\n\n✓ 基础语法\n✓ GPU 计算\n✓ 粒子系统\n✓ 碰撞检测\n✓ 交互效果\n✓ 网格动画\n\n下一步：尝试 Three.js、WebGL 或深入学习计算机图形学！`
