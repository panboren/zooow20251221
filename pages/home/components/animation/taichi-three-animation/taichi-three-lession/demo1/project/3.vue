<template>
  <div class="page-content">
    <div class="container">
      <h1>🎨 专业图片美化工具</h1>
      <p class="subtitle">GPU 加速 · 17 种高级滤镜 · 电影级算法</p>
...
async function createTexture(img: HTMLImageElement) {
  try {
    ...
    modesField = ti.field(ti.i32, [17])
    const modesArray = new Array(17).fill(0)
    selectedModes.value.forEach(id => {
      if (id >= 0 && id < 17) modesArray[id] = 1
    })
  ...
  renderKernel = ti.kernel(function render(time: any) {
    ...
    for (let f of ti.inputFragments()) {
      ...
      if (mode0 === 1) {
        ...
        // 只对边缘进行锐化
        let edgeStrength: f32 = ti.abs(center.r - up.r) + ti.abs(center.r - down.r) + ti.abs(center.r - left.r) + ti.abs(center.r - right.r)
        edgeStrength = edgeStrength / 4.0

        let strength: f32 = intensityVal * 0.5 * (0.3 + edgeStrength)
      ...
      if (mode4 === 1) {
        ...
        let exposure = 1.0 + intensityVal * 0.5
      ...
      if (mode5 === 1) {
        ...
        mixFactor = ti.max(0.0, ti.min(1.0, mixFactor))

        let tintR: f32 = tealR * (1.0 - mixFactor) + orangeR * mixFactor
        let tintG: f32 = tealG * (1.0 - mixFactor) + orangeG * mixFactor
        let tintB: f32 = tealB * (1.0 - mixFactor) + orangeB * mixFactor
