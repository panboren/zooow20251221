 ...
 * 
 * 创作灵感：
 * - 融合天体子午线的发光网络 + 量子涡旋的分形递归
 * - 超越级视觉：黑洞奇点 + 分形涡旋 + 能量球壳 + 粒子系统
 * - 电影级运镜：轨道环绕、俯冲穿越、螺旋上升
 * 
 * 核心特性：
 * 1. 中心黑洞奇点（TorusKnot + 分形Shader）
 * 2. 5层发光能量壳（同心球，Simplex噪声驱动）
 * 3. 4层分形涡旋环（递归自相似）
 * 4. 12000个能量粒子（轨道运动）
 * 5. 双层经纬网络（天体坐标）
 * 6. 3个旋转能量环（三色脉冲）
...
export default async function animateQuantumFractalVortex(props = {}, callbacks = {}) {
  ...
  try {
    ...
    // ==================== ⚫ 中心黑洞奇点 ====================
    const singularityGeometry = new THREE.TorusKnotGeometry(18, 6, 250, 40, 2, 5)
    const singularityMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          vec3 pos = position;
          float noise = snoise(vec3(pos * 0.3 + uTime * 0.4));
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // 黑洞边缘发光
          vec3 edgeColor = vec3(0.5, 0.0, 1.0);
          color += edgeColor * fresnel * (0.5 + pulse * 0.5);

          // 事件视界
          float eventHorizon = smoothstep(0.0, 0.3, fresnel);
          color += vec3(0.2, 0.6, 1.0) * eventHorizon * pulse * 0.3;

          float alpha = fresnel * uOpacity * (0.8 + pulse * 0.2);
    ...
    const shellColors = [0x9900ff, 0x6600ff, 0x3300ff, 0x0066ff, 0x00ffff]
      ...
      const material = new THREE.ShaderMaterial({
        ...
        vertexShader: `
          ...
          void main() {
            ...
            vec3 pos = position;
            float noise = snoise(vec3(pos * 0.15 + uTime * 0.25 + uLayer));
            pos += normal * noise * (2.0 + uLayer * 0.6);
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            float pulse = sin(uTime * 2.5 + uLayer * 1.256) * 0.5 + 0.5;
            vec3 color = uColor * (1.0 + pulse * 0.7);
            color += fresnel * uColor * 0.6;

            float alpha = fresnel * uOpacity * (0.25 + pulse * 0.15);
    ...
    const vortexMaterials = []
    for (let i = 0; i < 4; i++) {
      const radius = 50 + i * 25
      const geometry = new THREE.TorusGeometry(radius, 1.5, 16, 100)
      const material = new THREE.ShaderMaterial({
        uniforms: {
          uTime: { value: 0 },
        ...
        vertexShader: `
          ...
          void main() {
            ...
            vec3 pos = position;
            float noise = snoise(vec3(pos * 0.1 + uTime * 0.5 + uLayer));
            pos += normal * noise * (2.0 + uLayer * 0.5);
        ...
        fragmentShader: `
          ...
          void main() {
            float pulse = sin(uTime * 3.0 + vUv.x * 20.0 + uLayer * 1.57) * 0.5 + 0.5;
            vec3 color = uColor * (1.0 + pulse * 0.8);
            
            // 分形边缘
            float dist = length(vPosition.xy) / 120.0;
            float fractal = sin(dist * 50.0 - uTime * 4.0) * 0.5 + 0.5;
            color *= (0.8 + fractal * 0.4);

            float alpha = uOpacity * (0.5 + pulse * 0.3);
            gl_FragColor = vec4(color, alpha);
          }
        `,
        transparent: true,
      ...
      vortex.rotation.y = (i / 4) * Math.PI
      vortexGroup.add(vortex)
      vortexMaterials.push(material)
    }
    ...
    const innerGridMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float alpha = uOpacity * (0.35 + 0.2 * sin(uTime * 3.5 + vPosition.y * 0.1));
          gl_FragColor = vec4(baseColor * 1.4, alpha);
        }
      `,
      transparent: true,
      depthWrite: false,
    ...
    const outerGridMaterial = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          vec3 baseColor = vec3(0.7, 0.0, 1.0);
          float alpha = uOpacity * (0.25 + 0.15 * sin(uTime * 2.5 + vPosition.y * 0.05));
          gl_FragColor = vec4(baseColor * 1.2, alpha);
        }
      `,
      transparent: true,
      depthWrite: false,
    ...
    const innerGrid = createSphereGrid(60, innerGridMaterial, 36, 18)
    const outerGrid = createSphereGrid(85, outerGridMaterial, 52, 26)
    ...
    const energyRingGeometry = new THREE.TorusGeometry(100, 3.5, 16, 100)
      ...
      const ringMaterial = new THREE.ShaderMaterial({
        ...
        vertexShader: `
          ...
          void main() {
            vUv = uv;
            vec3 pos = position;
            float wave = sin(uTime * 5.0 + vUv.x * 35.0 + uOffset * 2.094) * 3.0;
            pos += normal * wave;
            gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0);
          }
        `,
        fragmentShader: `
          ...
          void main() {
            float pulse = sin(uTime * 6.0 + vUv.x * 18.0 + uOffset * 2.094) * 0.5 + 0.5;
            vec3 color = uColor * (1.0 + pulse * 1.2);
            float alpha = uOpacity * (0.65 + pulse * 0.45);
            gl_FragColor = vec4(color, alpha);
          }
        `,
        transparent: true,
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const hue = (i / particleCount) * 0.5 + 0.45
      const color = new THREE.Color().setHSL(hue, 1.0, 0.65)
      particleColors[i * 3] = color.r
      particleColors[i * 3 + 1] = color.g
      particleColors[i * 3 + 2] = color.b

      particleSizes[i] = 2 + Math.random() * 6
      particleSpeeds[i] = 0.25 + Math.random() * 1.5
    }
    ...
    const particleMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          vec3 pos = position;
          float orbit = sin(uTime * size * 0.07 + position.y * 0.025) * 5.0;
          pos.y += orbit;

          vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
          float pulse = sin(uTime * 5.0 + size) * 0.5 + 0.5;
          gl_PointSize = size * (500.0 / -mvPosition.z) * (0.65 + 0.55 * pulse);
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 finalColor = vColor * 1.7;
          gl_FragColor = vec4(finalColor, texColor.a * uOpacity * 0.85);
        }
      `,
      transparent: true,
      depthWrite: false,
    ...
    const pulseMaterialBase = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float alpha = smoothstep(dist, dist + 0.04, 1.0) * smoothstep(dist + 0.18, dist + 0.04, 1.0);
          vec3 color = vec3(0.3, 0.0, 1.0) * (1.0 - dist) + vec3(0.0, 0.8, 1.0) * dist;
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        if (onComplete) onComplete({ type: 'quantum-fractal-vortex' })
    ...
    // 阶段1: 黑洞奇点渐入 (0-2s)
    tl.to(singularityMaterial.uniforms.uOpacity, {
      value: 1,
      duration: 2,
      ease: 'power2.out'
    }, 0)
    ...
    function animate() {
      ...
      singularity.rotation.x += 0.005
      singularity.rotation.y += 0.008
      ...
      innerGrid.rotation.y += 0.0012
      outerGrid.rotation.y -= 0.0009

      // 更新能量环
      energyRings.forEach((ring, i) => {
        ring.rotation.x += 0.007 * (i + 1)
        ring.rotation.y += 0.012 * (i + 1)
      ...
      for (let i = 0; i < particleCount; i++) {
        const i3 = i * 3
        const angle = time * particleSpeeds[i] * 0.045
